import numpy as np
import scipy
import lmfit
from Acoustic.CONST import acoustic_speed, air_rho


class AbsorptionOpt:
    """
     █████╗ ██████╗ ███████╗ ██████╗ ██████╗ ██████╗ ████████╗██╗ ██████╗ ███╗   ██╗     ██████╗ ██████╗ ████████╗██╗███╗   ███╗██╗███████╗███████╗
    ██╔══██╗██╔══██╗██╔════╝██╔═══██╗██╔══██╗██╔══██╗╚══██╔══╝██║██╔═══██╗████╗  ██║    ██╔═══██╗██╔══██╗╚══██╔══╝██║████╗ ████║██║╚══███╔╝██╔════╝
    ███████║██████╔╝███████╗██║   ██║██████╔╝██████╔╝   ██║   ██║██║   ██║██╔██╗ ██║    ██║   ██║██████╔╝   ██║   ██║██╔████╔██║██║  ███╔╝ █████╗
    ██╔══██║██╔══██╗╚════██║██║   ██║██╔══██╗██╔═══╝    ██║   ██║██║   ██║██║╚██╗██║    ██║   ██║██╔═══╝    ██║   ██║██║╚██╔╝██║██║ ███╔╝  ██╔══╝
    ██║  ██║██████╔╝███████║╚██████╔╝██║  ██║██║        ██║   ██║╚██████╔╝██║ ╚████║    ╚██████╔╝██║        ██║   ██║██║ ╚═╝ ██║██║███████╗███████╗
    ╚═╝  ╚═╝╚═════╝ ╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝        ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚═══╝     ╚═════╝ ╚═╝        ╚═╝   ╚═╝╚═╝     ╚═╝╚═╝╚══════╝╚══════╝
    """

    @staticmethod
    def dbMiki_Residual(para: lmfit.Parameters, freq: np.ndarray, obj_data: np.ndarray = None):
        """
        DB Miki model, predict the normal incidence absorption coefficient.
        :param para: A lmfit.Parameters object, with the parameter to be optimized
        :param freq: The frequency of the absorption coefficient
        :param obj_data: The objective absorption coefficient, ** Normal incidence
        :return: The residual of the predict value and truth value
        """
        # Wrap the Objective Value
        val_dict = para.valuesdict()
        flow_res = val_dict["flow_res"]
        thick = val_dict["thick"]

        # DB_Miki Model Core
        # air impedance
        z_0 = acoustic_speed * air_rho
        # freq to flow_res ratio
        f_r = 1e3 * (freq / flow_res)
        # angular freq
        omega = 2 * np.pi * freq
        # Characristic impedance
        z_c = z_0 * (1 + 5.5 * f_r ** (-0.632) - 1j * 8.43 * f_r * (-0.623))
        # Complex wave number
        k_c = (omega / acoustic_speed) * (1 + 7.81 * f_r ** (-0.618) - 1j * 11.41 * f_r ** (-0.618))
        # Surface impedance
        z_s = -1j * z_c / np.tan(k_c * thick)
        # Normal incidence absorption coefficient
        alpha_0 = 1 - (np.abs((z_s - z_0) / (z_s + z_0))) ** 2

        return obj_data - alpha_0

    @staticmethod
    def dbMiki(flow_res: float, thick: float):
        """
        Use dbMiki model to predict the normal incidence absorption
        :param flow_res:
        :param thick:
        :param freq:
        :return:
        """
        # DB_Miki Model Core
        freq = np.array([125, 250, 500, 1e3, 2e3, 4e3])
        # air impedance
        z_0 = acoustic_speed * air_rho
        # freq to flow_res ratio
        f_r = 1e3 * (freq / flow_res)
        # angular freq
        omega = 2 * np.pi * freq
        # Characristic impedance
        z_c = z_0 * (1 + 5.5 * f_r ** (-0.632) - 1j * 8.43 * f_r * (-0.623))
        # Complex wave number
        k_c = (omega / acoustic_speed) * (1 + 7.81 * f_r ** (-0.618) - 1j * 11.41 * f_r ** (-0.618))
        # Surface impedance
        z_s = -1j * z_c / np.tan(k_c * thick)
        # Reflection coe
        r = (z_s - z_0) / (z_s + z_0)
        # Normal incidence absorption coefficient
        alpha_0 = 1 - (np.abs(r)) ** 2

        return alpha_0, r

    @staticmethod
    def jcaModel_Residual(para: lmfit.Parameters, freq: np.ndarray, obj_data: np.ndarray = None):
        """
        DB Miki model, predict the normal incidence absorption coefficient.
        :param para: A lmfit.Parameters object, with the parameter to be optimized
        :param freq: The frequency of the absorption coefficient
        :param obj_data: The objective absorption coefficient, ** Normal incidence
        :return: The residual of the predict value and truth value
        """
        # Wrap the Objective Value
        val_dict = para.valuesdict()
        tort = val_dict["tort"]
        flow_res = val_dict["flow_res"]
        porous = val_dict["porous"]
        viscous = val_dict["viscous"]
        thermal = val_dict["thermal"]
        thick = val_dict["thick"]

        # Some Const
        z_0 = air_rho * acoustic_speed
        eta = 1.84e-5  # Visocity of air [Poiseuille] (1.84*10^-5)
        omega = 2 * np.pi * freq  # Angular frequency
        gamma = 1.4  # Ratio of the specific heat capacity [-]
        npr = 0.71  # Prandtl number (0.77 at 20*C)
        p_0 = 101320  # Atmospheric pressure [N/m2]

        rho_eq = (tort * air_rho / porous) * (1 + ((flow_res * porous) / (1j * omega * air_rho * tort)) * np.sqrt(
            1 + (4 * 1j * (tort ** 2) * eta * air_rho * omega) / ((flow_res * viscous * porous) ** 2)))
        k_eq = (gamma * p_0 / porous) * ((gamma - (gamma - 1) / (
                1 + ((8 * eta) / (1j * thermal ** 2 * npr * omega * air_rho)) * np.sqrt(
            1 + (1j * air_rho * omega * npr * thermal ** 2) / (16 * eta)))) ** (-1))

        # Characristic impedance
        z_c = np.sqrt(rho_eq * k_eq)
        # Complex wave number
        k_c = np.sqrt(rho_eq / k_eq) * omega
        # Surface impedance of sample
        z_s = -1j * z_c * (1 / np.tan(k_c * thick))
        # Normalized specific acoustic impedance
        z_n = z_s / z_0
        # Reflection coefficient
        r = (z_s - z_0) / (z_s + z_0)
        # normal absorption coefficient
        normal_abs = 1 - np.abs(r) ** 2

        return normal_abs - obj_data

    @staticmethod
    def jcaModel(tort: float, flow_res: float, porous: float, viscous: float, thermal: float, thick: float):
        """
        Use JCA Model to compute the normal incidence absorption
        :param tort: Tortuosity [-] range[1-3]
        :param flow_res: Airflow resistivity [Ns/m4]
        :param porous: Porosity [-] range [0-1]
        :param viscous: Viscous characteristic dimension [m] [range 10-1000 10^-6m]
        :param thermal: Thermal characteristic dimension [m] [range 10-1000 10^-6m]
        :param thick: thickness [m]
        :return:
        """
        # Some Const
        freq = np.array([125, 250, 500, 1e3, 2e3, 4e3])
        z_0 = air_rho * acoustic_speed
        eta = 1.84e-5  # Visocity of air [Poiseuille] (1.84*10^-5)
        omega = 2 * np.pi * freq  # Angular frequency
        gamma = 1.4  # Ratio of the specific heat capacity [-]
        npr = 0.71  # Prandtl number (0.77 at 20*C)
        p_0 = 101320  # Atmospheric pressure [N/m2]

        rho_eq = (tort * air_rho / porous) * (1 + ((flow_res * porous) / (1j * omega * air_rho * tort)) * np.sqrt(
            1 + (4 * 1j * (tort ** 2) * eta * air_rho * omega) / ((flow_res * viscous * porous) ** 2)))
        k_eq = (gamma * p_0 / porous) * ((gamma - (gamma - 1) / (
                1 + ((8 * eta) / (1j * thermal ** 2 * npr * omega * air_rho)) * np.sqrt(
            1 + (1j * air_rho * omega * npr * thermal ** 2) / (16 * eta)))) ** (-1))

        # Characristic impedance
        z_c = np.sqrt(rho_eq * k_eq)
        # Complex wave number
        k_c = np.sqrt(rho_eq / k_eq) * omega
        # Surface impedance of sample
        z_s = -1j * z_c * (1 / np.tan(k_c * thick))
        # Normalized specific acoustic impedance
        z_n = z_s / z_0
        # Reflection coefficient
        r = (z_s - z_0) / (z_s + z_0)
        # normal absorption coefficient
        normal_abs = 1 - np.abs(r) ** 2

        return normal_abs, r

    @staticmethod
    def normal2diffuse(normal_alpha: float, *args):
        """
        The object function use to solve the nonlinear equation.
        :param normal_alpha: The normal incidence absorption, the param to optimize.
        :param args:
        :return:
        """
        reflection = np.sqrt(1 - normal_alpha)
        diffuse_alpha = args[0]
        return (2 / (1 - reflection) - (1 - reflection) / 2 + 2 * np.log((1 - reflection) / 2)) * 8 * (
                (1 - reflection) / (1 + reflection)) ** 2 - diffuse_alpha

    def fsolve_opt(self, diffuse_alpha: float):
        """
        Use scipy fsolve method to solve the normal2diffuse function, No Boundary
        :param diffuse_alpha: The absorption coefficient measure in diffuse field
        :return: The optima normal incidence absorption coefficient, and the residual of the object function
        """
        # Optimize the function
        normal_alpha = scipy.optimize.fsolve(self.normal2diffuse, diffuse_alpha, args=diffuse_alpha)
        # Compute the residual of the function
        res = self.normal2diffuse(normal_alpha[0], diffuse_alpha)
        return normal_alpha, res

    def brentq_opt(self, diffuse_alpha: float):
        """
        Use brenqt algorithm to sovle the normal2diffuse function with the boundary of (0,1)
        :param diffuse_alpha: The absorption coefficient measure in diffuse field
        :return: The optima normal incidence absorption coefficient, and the residual of the object function
        This is Bad Method！！！！！！！！
        """
        # Optimize the function with Bound
        normal_alpha = scipy.optimize.brentq(self.normal2diffuse, 0, 1, args=(diffuse_alpha,))
        # Compute the residual of the function
        res = self.normal2diffuse(normal_alpha, diffuse_alpha)
        return normal_alpha, res

    def dbmiki_opt(self, normal_alpha: np.ndarray):
        """
        Use lmfit optimizer to fit the curve
        :param normal_alpha: the 
        :return:
        """
        assert normal_alpha.shape == (6,)
        # Construct Parameter
        para = lmfit.Parameters()
        para.add('flow_res', value=2e5, min=2e3, max=8e5)
        para.add('thick', value=0.1, min=0.01, max=1)

        # Frequency to optimize
        freq = np.array([125, 250, 500, 1e3, 2e3, 4e3])

        # Construct optimizer
        output = lmfit.minimize(self.dbMiki_Residual, para, args=(freq,), kws={'obj_data': normal_alpha},
                                method='nelder', nan_policy='omit')

        # Wrap the output
        flow_res = output.params.valuesdict()['flow_res']
        thick = output.params.valuesdict()['thick']

        return flow_res, thick, output

    def jca_opt(self, normal_alpha: np.ndarray):
        """
        Use JCA model to compute minimize the L2 norm of predict subtract real, get 5 params
        :param normal_alpha: The real data of absorption coe
        :return: A tuple of 6 result
        tort: Tortuosity [-] range[1-3]
        flow_res: Airflow resistivity [Ns/m4]
        porous: Porosity [-] range [0-1]
        viscous: Viscous characteristic dimension [m] [range 10-1000 10^-6m]
        thermal: Thermal characteristic dimension [m] [range 10-1000 10^-6m]
        thick: thickness [m]
        output: lmfit output object
        """
        assert normal_alpha.shape == (6,)
        # Construct Parameter
        para = lmfit.Parameters()
        para.add('tort', value=2, min=1, max=3)
        para.add('flow_res', value=2e5, min=2e3, max=8e5)
        para.add('porous', value=0.3, min=0, max=1)
        para.add('viscous', value=1e-4, min=1e-6, max=1e-3)
        para.add('thermal', value=1e-4, min=1e-6, max=1e-3)
        para.add('thick', value=0.1, min=0.01, max=1)

        # Frequency to optimize
        freq = np.array([125, 250, 500, 1e3, 2e3, 4e3])

        # Construct optimizer
        output = lmfit.minimize(self.jcaModel_Residual, para, args=(freq,), kws={'obj_data': normal_alpha},
                                method='nelder', nan_policy='omit')

        # Wrap the output
        tort = output.params.valuesdict()['tort']
        flow_res = output.params.valuesdict()['flow_res']
        porous = output.params.valuesdict()['porous']
        viscous = output.params.valuesdict()['viscous']
        thermal = output.params.valuesdict()['thermal']
        thick = output.params.valuesdict()['thick']

        return tort, flow_res, porous, viscous, thermal, thick, output

    def retrieve_impedance(self, diffuse_alpha: np.ndarray, **kwargs):
        """
        Retrieve the normal incidence impedance from the diffuse alpha
        :param diffuse_alpha: The absorption coe test in diffuse field
        :param kwargs:
        solve_method: For the equation solve method, from diffuse to normal alpha,
                        2 options, ['fsolve', 'brentq'], 'fsolve' is highly recommend
        opt_model: For the normal incidence model to be used in optimization,
                        2 options, ['dbMiki', 'JCA'], 'JCA' is highly recommend
        :return: normal_alpha, predict_normal_alpha, predict_normal_impedance, func_res, model_res, model_param
        """
        # Check input
        assert diffuse_alpha.shape == (6,)
        func_solve_method = kwargs['solve_method']
        opt_model = kwargs['opt_model']
        assert func_solve_method in ['fsolve', 'brentq']
        assert opt_model in ['dbMiki', 'JCA']

        # Convert diffuse alpha into normal incidence alpha
        normal_alpha = []
        func_res = []
        if func_solve_method == 'brentq':
            for i in diffuse_alpha:
                tmp_normal_alpha, tmp_func_res = self.brentq_opt(i)
                normal_alpha.append(tmp_normal_alpha[0])
                func_res.append(tmp_func_res)
        else:
            for i in diffuse_alpha:
                tmp_normal_alpha, tmp_func_res = self.fsolve_opt(i)
                normal_alpha.append(tmp_normal_alpha[0])
                func_res.append(tmp_func_res)
        normal_alpha = np.array(normal_alpha)
        func_res = np.array(func_res)

        # Opt the normal incidence model
        if opt_model == 'JCA':
            # Use JCA model to retrieve the impedance
            tort, flow_res, porous, viscous, thermal, thick, output = self.jca_opt(normal_alpha)
            predict_normal_alpha, predict_normal_impedance = self.jcaModel(tort, flow_res, porous, viscous, thermal,
                                                                           thick)
            model_res = normal_alpha - predict_normal_alpha
            model_param = np.array([tort, flow_res, porous, viscous, thermal, thick])

        else:
            # Use Miki model to retrieve the impedance
            flow_res, thick, output = self.dbmiki_opt(normal_alpha)
            predict_normal_alpha, predict_normal_impedance = self.dbMiki(flow_res, thick)
            model_res = normal_alpha - predict_normal_alpha
            model_param = np.array([flow_res, thick])

        return normal_alpha, predict_normal_alpha, predict_normal_impedance, func_res, model_res, model_param


if __name__ == '__main__':
    import matplotlib.pyplot as plt

    # Use Chinese Require
    plt.rcParams["font.sans-serif"] = ["SimHei"]
    plt.rcParams["axes.unicode_minus"] = False

    # obj_alpha = np.array([0.20, 0.65, 0.80, 0.92, 0.80, 0.85])
    # obj_alpha = np.array([0.23, 0.42, 0.83, 0.93, 0.74, 0.83])
    # obj_alpha = np.array([0.26, 0.13, 0.08, 0.06, 0.06, 0.06])
    obj_alpha = np.array([0.01, 0.01, 0.02, 0.02, 0.02, 0.02])
    normal_alpha, predict_normal_alpha_JCA, predict_normal_impedance, func_res, model_res, model_param = AbsorptionOpt().retrieve_impedance(
        obj_alpha, solve_method='fsolve', opt_model='JCA')
    print(f'normal_alpha: {normal_alpha}')
    print(f'predict_normal_alpha: {predict_normal_alpha_JCA}')
    print(f'predict_normal_impedance: {predict_normal_impedance}')
    print(f'diffuse to normal res: {func_res}')
    print(f'optimization residual: {model_res}')
    print(f'model param is: {model_param}')
    print('======================================')

    normal_alpha, predict_normal_alpha_DB, predict_normal_impedance, func_res, model_res, model_param = AbsorptionOpt().retrieve_impedance(
        obj_alpha, solve_method='fsolve', opt_model='dbMiki')
    print(f'normal_alpha: {normal_alpha}')
    print(f'predict_normal_alpha: {predict_normal_alpha_DB}')
    print(f'predict_normal_impedance: {predict_normal_impedance}')
    print(f'diffuse to normal res: {func_res}')
    print(f'optimization residual: {model_res}')
    print(f'model param is: {model_param}')
    print('======================================')

    # Plot Data
    plt.plot([0, 1, 2, 3, 4, 5], obj_alpha, linewidth=1, marker='s', label=r'扩散场吸声 $\alpha_{d}$')
    plt.plot([0, 1, 2, 3, 4, 5], normal_alpha, linewidth=1,marker='s', label=r'垂直入射吸声 $\alpha_{0}$')
    plt.plot([0, 1, 2, 3, 4, 5], predict_normal_alpha_JCA,  linewidth=1,marker='s', label=r'JCA吸声 $\alpha_{0}$')
    plt.plot([0, 1, 2, 3, 4, 5], predict_normal_alpha_DB,  linewidth=1,marker='s', label=r'DB_Miki吸声 $\alpha_{0}$')
    plt.xticks([0, 1, 2, 3, 4, 5], ['125', '250', '500', '1k', '2k', '4k'])
    plt.ylim(0, 0.2)
    plt.xlabel('频率（Hz）', fontsize=12)
    plt.ylabel(r'吸声系数$\alpha$（-）', fontsize=12)
    plt.legend()
    plt.grid()
    plt.show()
